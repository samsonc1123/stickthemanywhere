...
// Supports high-intelligence pet profiles (Capuchins, Chimps).
...
const primateSafetyValidator = v.object({
 ...
 *
 * Args:
 *   speciesId           — taxonomy node id for the plant/fruit/species.
 *   highIntelligencePets — when true, appends primateSafety warnings
 *                          to the response if the protocol has them.
 *
 * Returns null if the species does not exist.
 * Returns a safe-default payload with protocolMissing: true if no
 * safetyProtocol has been authored yet.
 */
export const getSafetyAlert = query({
  args: {
    speciesId: v.id("taxonomy"),
    highIntelligencePets: v.optional(v.boolean()),
  },
  handler: async (ctx, args) => {
    ...
    // Only include primateSafety block when the user profile has
    // high-intelligence pets enabled AND the protocol has the data.
    const primateSafety =
      args.highIntelligencePets === true
        ? (protocol.primateSafety ?? null)
        : null;
 ...
 *
 * primateSafety is optional — only set it for species where
 * Capuchin / Chimp access is a realistic risk.
 */
export const upsertSafetyProtocol = mutation({
  args: {
    ...
    primateSafety:   v.optional(primateSafetyValidator),
  },
  handler: async (ctx, args) => {
    const now = Date.now();
    ...
    if (existing !== null) {
      await ctx.db.patch(existing._id, {
        ...
        primateSafety:   args.primateSafety,
        updatedAt:       now,
      });
