...
export default function HispanicPage() {
  // Fetch flag stickers from database
  const { data: flagStickers = [], isLoading } = useQuery<any[]>({
    queryKey: [`/api/stickers/category/Hispanic/subcategory/Flags`],
  });
        ...
        <div className="flex justify-center pb-4 landscape:pb-16">
          <div className="grid grid-cols-1 landscape:grid-cols-2 gap-3 landscape:gap-4 max-w-md landscape:max-w-4xl px-4">
            <div
              ...
              data-testid="sticker-box-hispanic"
            >
              <div 
              ...
              >
                <div className="flex gap-2 px-2">
                  {isLoading ? (
                  ...
                  ) : flagStickers.length > 0 ? (
                    flagStickers.map((sticker: any) => (
                      <img 
                        key={sticker.id}
                    ...
                    <span className="text-center w-full text-orange-500">Hispanic Stickers</span>
                  )}
                </div>
              </div>
            </div>
