...
const SUBCATEGORIES = [
  { name: "Adventure Time",     code: "ADV" },
  { name: "Baby Shark",         code: "BAB" },
  { name: "Bob's Burgers",      code: "BOB" },
  { name: "Beavis & Butt-Head", code: "BOD" },
  { name: "Family Guy",         code: "FGY" },
  { name: "Futurama",           code: "FUT" },
  { name: "King of the Hill",   code: "KOH" },
  { name: "Regular Show",       code: "REG" },
  { name: "Rick & Morty",       code: "RAM" },
  { name: "The Simpsons",       code: "SIM" },
  { name: "South Park",         code: "SPA" },
  { name: "Steven Universe",    code: "STU" },
];
const ACCENT = "#f9a8d4";
const CAT = "ANIMATED-SERIES";
  ...
  return (
    <div className="min-h-screen bg-perforated text-white font-orbitron flex flex-col items-center p-4 pt-4 landscape:pt-2 pb-16">
      ...
      <div className="text-center mb-4 lg:mb-1">
        <h1 className="font-bold animate-categoriesFlicker font-audiowide text-lg" style={{ color: ACCENT }}>Animated Series</h1>
      </div>
            ...
            {SUBCATEGORIES.map(sub => {
              ...
              return (
                <button key={sub.code} onClick={() => setActiveCode(activeCode === sub.code ? null : sub.code)}
                  ...
                  style={{ backgroundColor: ACCENT, color: "black", boxShadow: lit ? `0 0 8px ${ACCENT}, 0 0 16px ${ACCENT}88` : "none", transition: "box-shadow 0.4s" }}>
                  {sub.name}
                </button>
              );
            })}
          ...
          {isLoading ? [...Array(4)].map((_, i) => (
            ...
            return (
              <div key={sub.code} className="w-52 h-52 landscape:w-52 landscape:h-52 md:w-56 md:h-56 md:landscape:w-56 md:landscape:h-56 border-4 overflow-hidden relative"
                style={{ borderColor: ACCENT, boxShadow: lit ? `0 0 12px ${ACCENT}, 0 0 24px ${ACCENT}55, inset 0 0 10px ${ACCENT}22` : `0 0 5px ${ACCENT}44`, transition: "box-shadow 0.4s" }}>
                {stickers.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center gap-2 w-full h-full px-3 text-center">
                    <span className="text-xs font-montserrat font-bold leading-tight" style={{ color: ACCENT }}>{sub.name}</span>
